...
httpVueLoader.load = function(url, name) {
  ...
  function normalizeContent(elt) {
    ...
    if ( !elt.hasAttribute('src') ) {
      if (window['CoffeeScript'] && /coffeescript/i.test(elt.getAttribute('lang') || elt.getAttribute('type') || '')) {
        try {
          elt.innerHTML = CoffeeScript.compile(elt.innerHTML, {bare: true});
        } catch (err) {
          console.error(err);
        }
      }
    ...
    return httpVueLoader.httpRequest(elt.getAttribute('src')).then( function(content) {
      if (window['CoffeeScript'] && /\.coffee$/i.test(elt.getAttribute('src'))) {
        try {
